...
| `完整纯净源码` | 零代码混淆，保留可读可维护的完整工程代码 |

**安装前请确认**：

- 手机是 **arm64** 架构（现在的手机基本都是）；
- 系统是 **Android 10 以上**（API 29+，完美自适应 Android 10 ~ Android 15）；
- 至少留出 **2.1GB 可用空间**（首创流式直解免落盘架构，无需在闪存缓存临时分片，解包更省空间更快速）。
...
2. 改 `pubspec.yaml` 的版本名（`<版本>+<构建号>` 里 **`+` 前面那部分**，
   例如 `1.1.1+20260920` → `1.1.2+20260920`）；
3. 在 `CHANGELOG.md` 加一节同名标题，写清楚改了什么；
4. 推送到 `main`，剩下的交给 CI。

> **`versionCode` 由 CI 自动管理，不用手改。**
> CI 取「`30000000 + 工作流运行号`」作为 versionCode，保证每次构建都严格递增，
> 这样用户永远可以直接覆盖安装。`pubspec.yaml` 里 `+` 后面的数字只作记录用途，
> 不再参与 versionCode 计算（历史上用过它，值约 2026xxxx，新方案已高于该区间）。
>
> 上限 2100000000 是 Android 的硬限制，本方案余量约 20 亿次构建，无需担心。
