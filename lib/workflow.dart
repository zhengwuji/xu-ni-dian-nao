...
class Util {
  ...
  //TINY-OPT: copyAsset 漏了 offsetInBytes/lengthInBytes，copyAsset2 才是正确写法。
  //这里加一层带长度校验的封装，首启复制用；顺带做断点续跑的跳过与校验。
  static Future<void> copyAssetVerified(String src, String dst) async {
    ByteData data = await rootBundle.load(src);
    final int size = data.lengthInBytes;
    final f = File(dst);
    if (await f.exists() && await f.length() == size) {
      return; // 已复制完成，跳过（避免失败重试时重灌一遍 GB 级数据）
    }
    await f.writeAsBytes(data.buffer.asUint8List(data.offsetInBytes, size), flush: true);
    final int written = await f.length();
    if (written != size) {
      try { await f.delete(); } catch (_) {}
      throw StateError("复制 $src 失败：期望 $size 字节，实际 $written 字节");
    }
  }
  ...
  //TINY-OPT: 首启安装用的“执行并检查退出码”版本。原实现处处丢弃 exitCode，
  //安装失败时界面仍然显示“安装完成”。
  static Future<void> executeChecked(String script, String stage) async {
    final int code = await execute(script);
    if (code != 0) {
      throw StateError("$stage 失败（退出码 $code）");
    }
  }
...
class Workflow {
  ...
  //全部丢弃，脚本自身也没有 set -e，任何一步失败都会被下一句覆盖掉。
...
#TINY-OPT: zip-slip 防护。原实现直接 unzip 到 DATA_DIR，包内若有 ../ 路径会写到应用私有目录之外。
\$DATA_DIR/bin/busybox mkdir -p \$DATA_DIR/tmp/unzip
\$DATA_DIR/bin/busybox unzip -o assets.zip -d \$DATA_DIR/tmp/unzip
\$DATA_DIR/bin/busybox cp -a \$DATA_DIR/tmp/unzip/bin/. \$DATA_DIR/bin/
\$DATA_DIR/bin/busybox rm -rf \$DATA_DIR/tmp/unzip
chmod -R +x bin/*
#TINY-OPT: 原写的 libexec/proot/* 在本工程的 assets.zip 里并不存在，恒报 No such file
[ -d "\$DATA_DIR/libexec" ] && chmod -R +x libexec/* || true
chmod 1777 tmp
...
if mv "\$CONTAINER_DIR" "\$DATA_DIR/containers/0"; then
	rm -rf "\$DATA_DIR/containers/0.old"
else
	if [ -d "\$DATA_DIR/containers/0.old" ]; then mv "\$DATA_DIR/containers/0.old" "\$DATA_DIR/containers/0"; fi
	exit 1
fi
...
"commands":${jsonEncode(Localizations.localeOf(G.homePageStateContext).languageCode == 'zh' ? D.commands : D.commands4En)}
  ...
  static Future<void> _ensureEnoughSpace(List<String> xaFiles) async {
    int shardBytes = 0;
    for (String name in xaFiles) {
      //用 AssetBundle.loadStructuredBinaryData 直接拿 Asset 的字节长度，不申请大块内存；
      //load() 会把整片（98MB）实体化到 Dart 堆里，只为量个大小不值得。
      await rootBundle.loadStructuredBinaryData("assets/$name", (ByteData data) async {
        shardBytes += data.lengthInBytes;
        return null;
      });
    }
    ...
    try {
      ...
      usable = null; // 旧版本 App 侧没有这个方法时不阻塞安装
    }
  ...
  static Future<void> initData() async {
    ...
    //如果没有这个key，说明是初次启动
    //TINY-OPT: 首启门闩原来只有 defaultContainer（它在安装完成后才写入），
    //进程若在安装中途被杀，下次会整轮重来并重拷 GB 级分片。这里改成看安装完成标记：
    //没装完就继续装（分片已有且大小正确会被跳过），装完了才落 defaultContainer。
    if (!G.prefs.containsKey("defaultContainer") ||
        !(G.prefs.getBool("containerInstalled") ?? false)) {
...
"""));
  ...
  static Future<void> launchCurrentContainer() async {
    ...
    if (Util.getGlobal("getifaddrsBridge")) {
      //TINY-OPT: 服务端已改为单实例（flock），重复启动会自行退出，这里无需探测。
      //原来每次启动容器都会再起一个，旧进程会变成孤儿永久阻塞在 accept。
